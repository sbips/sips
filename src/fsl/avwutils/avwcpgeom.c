...
void usage(void)
{
  printf("\nUsage: avwcpgeom <source> <destination> [-d]\n\n");
  printf("-d : don't copy image dimensions\n\n");
  exit(1);
}
...
/*   avw_read_hdr(argv[1],&hdr,&doswap); */
...
/*   avw_read_hdr(argv[2],&hdr2,&doswap); */
  ...
  /* NB: size of buffer needs to hold max size in datatype of dest, as this is
...
/* for(i=0;i<8;i++) */
/* { */
/*   hdr2.dime.pixdim[i]=hdr.dime.pixdim[i]; */
/*   if (copydim) */
