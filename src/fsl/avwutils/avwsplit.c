...
/*  avwsplit.c - split 4D Analyze file into 3D files for SPM
...
int main(int argc, char **argv)
{
  ...
  bufsz = x * y * z * (FslGetDataType(src, &t) / 8);
  buffer = (void *) calloc(bufsz, sizeof(char));
  ...
  for(i = 0; i < v; i++) {
    ...
    FslSetDim(dest, x, y, z, 1);
    FslWriteHeader(dest);
    FslWriteVolumes(dest, buffer, 1);
    FslClose(dest);
  }
