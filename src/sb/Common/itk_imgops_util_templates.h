...
#ifndef _itk_imgops_util_templates_h_
#define _itk_imgops_util_templates_h_
...
//!!!!!!!!!!! create new image here !!!!!!!!!!!!!!!!!!!!!!!!
  ...
  RelabelConnectedComponents( typename itk::Image< TInputPixelType, TDims>::Pointer input, int minObjectSize )
  {
	...
	threshold->SetInsideValue(itk::NumericTraits<TInputPixelType>::One);
	threshold->SetOutsideValue(itk::NumericTraits<TInputPixelType>::Zero);
    ...
    //connectedFilter->SetInput( threshold->GetOutput() );
	...
	{
		std::cout<<exp;
		std::cout<<connectedFilter->GetObjectCount();
		std::cout << static_cast<double>(itk::NumericTraits<TOutputPixelType>::max());
	}
