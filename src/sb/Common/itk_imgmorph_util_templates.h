...
#ifndef _itk_imgmorph_util_templates_h_
#define _itk_imgmorph_util_templates_h_
...
#include "itk_imgops_util_templates.h" // egitk namespace
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  DilateBinaryVol2D( typename TInputImageType::Pointer input, TStructElementType2D structElement )
  {
    ...
    return egitk::FilterVolAsSlice< InputPixelType, OutputPixelType >( input, dilateFilter );
  ...
  ErodeBinaryVol2D( typename TInputImageType::Pointer input, TStructElementType2D structElement )
  {
    ...
    return egitk::FilterVolAsSlice< InputPixelType, OutputPixelType >( input, erodeFilter );
...
/*
  ...
  GetObjectPerimeter( typename itk::Image< TPixelType, TDims>* input )
  {
    ...
    brain = egitk::ThresholdImageToBinary< TPixelType, TPixelType, TDims>( input, 1, 0, 1, 7 );
    brain = egitk::FillHolesInBinaryVol2D< TPixelType, TPixelType >( brain );

    typename ImageType::Pointer temp = ImageType::New();
    temp = egitk::DuplicateImage< TPixelType, TDims>( &*brain );
  
    temp = egitk::ErodeBinaryVol2D< ImageType, ImageType >( temp, egitk::CreateBallSE< double, 2 >( 1 ) );
    temp = egitk::ErodeBinaryVol2D< ImageType, ImageType >( temp, egitk::CreateBallSE< double, 2 >( 1 ) );
