...
//eg///////////////////////////////////////////////////////////////////////
  int version;
  obj_file.read(&version,sizeof(int));
...
//eg///////////////////////////////////////////////////////////////////////
  ...
  //eg///////////////////////////////////////////////////////////////////////
  if (swap)
    version = iconv(version);
  
    // ignore field F (number of object volumes) for analyze 8 or greater object maps 
  if (version >= 20050829)
     obj_file.seekg(sizeof(int)*6); 
  //eg///////////////////////////////////////////////////////////////////////
  ...
  for(z=0; z < sz[2]; ++z)
      for(y=0; y < sz[1]; ++y){
          ...
          case(LOP_LINE):
              ...
              else *pt = *(pt - 3*sz[1]);  //eg changed from:  else *pt = *(pt - 3*sz[0]);
              break;
          case(LOC_LINE):
...
//-----------------------------------------------------------------------------------
// temporal lobe subdivision 
// modified by eg
//-----------------------------------------------------------------------------------

int get_temporal_occipital_y_value( uchar* trace_mask, int TLvalue )
{
  for ( int z1=0; z1<sz[2]; ++z1 )
    for ( int y1=0; y1<sz[1]; ++y1 )
    {
      int index = z1*sz[1] + y1;
      if ( trace_mask[ index ] == TLvalue )
      {
        return y1 + 1;
      }
    }
...
void subdivide_temporal_lobe( uchar* trace_mask, int CSline, int SFline, int Tlobe, int TlobeNew, int pron, int TO_y_value )
{
    // Erase any part of the CSline that extends below SFline (temporarily)
   std::vector< std::pair<int,int> > CSline_removed_points; 
   bool search = true;
    for ( int z1=0; z1<sz[2] && search; ++z1 )
      for ( int y1=0; y1<sz[1] && search; ++y1 )
      {
        int index = z1 * sz[1] + y1;
        if ( trace_mask[ index ] == CSline )
          if ( trace_mask[ index-1 ] == SFline || trace_mask[ index+1 ] == SFline )
          {
            search = false;
          }
          else
          {
            trace_mask[ index ] = Tlobe;
            CSline_removed_points.push_back( std::pair<int,int>(y1,z1) );
          }
      }

      // Find top of temporal lobe demarcated by SFline
      std::vector< std::pair<int,int> > Tlobe_top;
      
      for ( int y1=TO_y_value; y1<sz[1]; ++y1 )
        for ( int z1=sz[2]-1; z1>=0; --z1 )
        {
            int index = z1 * sz[1] + y1;
            if ( trace_mask[ index ] == Tlobe )
            {
              Tlobe_top.push_back( std::pair<int,int>(y1,z1) );
              break;
            }
        }
        
        // Recode top of temporal lobe to subdivide area into 2 regions
        int distance = ( Tlobe_top[0].second - pron ) / 2;
        for ( int i=0; i < Tlobe_top.size(); ++i )
          for ( int d=0; d<=distance; ++d )
          {
              int index = (Tlobe_top[i].second - d) * sz[1] + Tlobe_top[i].first;
              if ( trace_mask[ index ] == Tlobe )
                trace_mask[ index ] = TlobeNew;
          }

      // Add back any part of the CSline that was removed
      for (int i=0; i<CSline_removed_points.size(); ++i )
      {
          int index = CSline_removed_points[i].second*sz[1] + CSline_removed_points[i].first;
          trace_mask[ index ] = CSline;
      }
