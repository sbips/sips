...
int main(int argc, char ** argv)
{
  ...
  // make right and left mask edges filled with gridline
   ...
   // LEFT (sagital stuff + temporal lobe subdivision fix)
   connect_trancings(left_trace_mask, 0, lpron);
   fill_trace_mask(left_trace_mask, 0, 0);
   // modified by eg
   int left_TO_y = get_temporal_occipital_y_value( left_trace_mask,105);      
   //
   recolor_lines(left_trace_mask, 0, 0);
   // modified by eg
   subdivide_temporal_lobe(left_trace_mask, 100,102,11,29,lpron,left_TO_y);
  //
       
  // RIGHT (sagital stuff + temporal lobe subdivision fix)
   connect_trancings(right_trace_mask, LINE_OFFSET, rpron);
   fill_trace_mask(right_trace_mask, LINE_OFFSET, LOBE_OFFSET);
  // modified by eg
   int right_TO_y = get_temporal_occipital_y_value( right_trace_mask,125);       
   //
   recolor_lines(right_trace_mask, LINE_OFFSET, LOBE_OFFSET);
   // modified by eg
   subdivide_temporal_lobe(right_trace_mask,120,122,24,30,rpron,right_TO_y);
   //
