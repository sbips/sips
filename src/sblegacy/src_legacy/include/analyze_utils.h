...
bool read_analyze_header(const string &hdr_fname, info &hdr_info)
{
    ...
    if(datatype != 2 && datatype != 4 && datatype != 32) {
      // assuming that 2 and 4 and 32 are only choices
      // must swap bytes
      swap = true;
    }
...
T *read_analyze(const string &froot, info &hdr_info)
{
    ...
    if(!(hdr_info.datatype == 2 || hdr_info.datatype == 4 || hdr_info.datatype == 32)) {
	...
	else if(Tinfo.datatype = 32 && swap) {
	  T *ptr = img;
	  for (int i=0; i < volume; i++, ptr++) *ptr = siconv(*ptr);
	}
        return(img);
